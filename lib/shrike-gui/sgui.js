...
export class SGui {
    ...
    createWindow = (title, isOpen, dockSide = null) => {
        ...
        // Determine which dock to use based on window title or provided dockSide
        if (dockSide) {
            if (dockSide === "left") {
                this.leftDockDrop(sguiWindow);
                sguiWindow.dockedTo = "left";
            } else if (dockSide === "right") {
                this.rightDockDrop(sguiWindow);
                sguiWindow.dockedTo = "right";
            }
        } else {
            // Default docking based on window title
            const leftDockWindows = ["Scene", "Resource Window", "Menu Bar"];
            const rightDockWindows = ["Properties", "Transformation", "Material", "Physics"];
            
            if (leftDockWindows.some(name => title.includes(name))) {
                this.leftDockDrop(sguiWindow);
                sguiWindow.dockedTo = "left";
            } else if (rightDockWindows.some(name => title.includes(name))) {
                this.rightDockDrop(sguiWindow);
                sguiWindow.dockedTo = "right";
            } else {
