...
export default class SGui {
    ...
    createDockingZone() {
        ...
        /**
         * leftDock.addEventListener("mouseup",()=>{
         *  leftDock.dispatchEvent(new CustomEvent("dock-drop",{docker:left}))
         * })
         *
         */
        return leftDock;
    ...
    leftDockDrop(e) {
        if (this.draggingWindow ) {
            // switch element to ul

        const windowElement = this.shadowRoot.getElementById(this.draggingWindow); // Get the window element by its ID
    ...
    handleWindowDragStart(e) {
        /*
         * this.draggingWindow = e.detail.windowId
         */
        const { windowId } = e.detail;
        ...
        const onMouseMove = (moveEvent) => {
            ...
            if (moveEvent.clientX < dockRect.right && !hasCrossedDockBoundary) {
                ...
                moveEvent.clientX >= dockRect.right &&
                hasCrossedDockBoundary
            ) {
        ...
        const onMouseUp = () => {
            ...
            if (hasCrossedDockBoundary && !windowElement.isDocked) {
                ...
                !hasCrossedDockBoundary &&
                (windowElement.isDocked || windowElement.isDocked === undefined)
            ) {
                if (this.leftDockContents.length === 1) {
