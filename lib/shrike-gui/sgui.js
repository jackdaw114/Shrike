...
export default class SGui {
    ...
    addEventListeners() {
        ...
        //         this.handleRightUndock(e.detail.window, true);
    ...
    handleWindowDragEnter(e) {
        ...
        console.log("Window (x) is now at " + this.draggingWindow.style.left + "px");
    ...
    rightDockDrop = (windowElement) => {
        if (windowElement) {
           ...
           if (!windowElement.isDocked) {
                ...
                // Resize the dock to match the window's width
                this.rightDock.style.width = `${windowElement.offsetWidth}px`;
...
export class SGuiWindowBase extends HTMLElement {
    ...
    appendSlider(controller,label,element) {
        let slider = new SGuiSlider(controller,label,element=element);
...
class SGuiSlider extends HTMLElement {
    static nextId = 0;
    constructor(sliderState, label, element = null) {
        super();
        this.label = label;
        ...
        labelDiv.textContent = this.label || `Slider ${this.sliderId}`;
