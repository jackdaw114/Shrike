export async function parseOBJ(objFile) {
    // If the input is a Blob or File, read it as text
    if (objFile instanceof Blob) {
        ...
        return parseOBJString(text);
    }
    // If it's already a string, parse it directly
    return parseOBJString(objFile);
}

function parseOBJString(objFile) {
    const positions = [];
    const texCoords = [];
    const normals = [];
    const indices = [];
    ...
    for (const line of lines) {
        ...
        // Parse faces (triangles)
            ...
            for (let i = 1; i < parts.length; i++) {
                ...
                const texCoordIndex = parseInt(indicesSet[1]) - 1 || 0;
                ...
                if (!vertexMap.has(vertexKey)) {
                    ...
                    vertices.push(
                        ...
                        texCoords[2 * texCoordIndex] || 0,     // u
                        texCoords[2 * texCoordIndex + 1] || 0, // v
                        0 //TexID change this
                    );

                    vertexMap.set(vertexKey, vertices.length /VERTEX_SIZE  - 1);
                }
